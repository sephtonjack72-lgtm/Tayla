...
1. Screens & Tabs
1. Modals
1. Consent & Settings Logic
1. FY (Financial Year) Management
1. Data Load & Persist
1. Tax Calculator
1. Budget Tracker
1. Financial Health
1. Goals
1. Data Rights (APP 12)
1. Utils
1. Init
   ═══════════════════════════════════════════════════════════ */
...
empty.innerHTML = `<div style="font-size:2rem;margin-bottom:12px">💰</div> <div style="font-size:1rem;font-weight:600;color:var(--ink-1);margin-bottom:8px">Add your first pay to get started</div> <div style="font-size:0.82rem;margin-bottom:20px;line-height:1.5">Enter your gross pay for any week below.<br>Tayla will calculate your take-home automatically.</div> <button class="btn btn-primary btn-sm" onclick="expandAndFocusCurrentWeek()">Enter first pay →</button>`;
...
row.innerHTML = ` <div class="wc wc-label">${label}</div> <div class="wc"><input class="wc-input" type="number" min="0" step="0.01" placeholder="0.00" id="wi${i}" value="${val1 ? val1 : ''}" oninput="onPeriodInput(${i})" onchange="onPeriodInput(${i})"></div> <div class="wc"><input class="wc-input" type="number" min="0" step="0.01" placeholder="0.00" id="wi2${i}" value="${val2 ? val2 : ''}" oninput="onPeriodInput2(${i})" onchange="onPeriodInput2(${i})"></div> <div class="wc wc-net ${combined ? '' : 'wc-empty'}" id="wn${i}">${combined ? fmt(combined) : '—'}</div>`;
...
APP_DATA.budget[budgetMonthIndex] = APP_DATA.budget[budgetMonthIndex]
.filter(e => e.autoTaxPeriod !== periodIndex + ‘*’ + TAX_MODE);
if (netVal > 0) {
APP_DATA.budget[budgetMonthIndex].push({
id: Date.now(),
...
cat: ‘Salary/Wages’,
date: new Date().toLocaleDateString(‘en-AU’),
autoTaxPeriod: periodIndex + ’*’ + TAX_MODE,
});
}
}
renderBudget();
...
return ` <div class="entry-item"> <div class="entry-cat-dot" style="background:${isDebt ? 'var(--red)' : (getCatColors()[e.cat]||'#999')}"></div> <div> <div class="entry-desc">${escHtml(e.desc)}${drawBadge}${debtBadge}</div> <span class="entry-cat-label">${catLabel} · ${e.date}</span> </div> <div class="entry-amount ${amtClass}">${prefix}${fmt(e.amount)}</div> <button class="entry-del" onclick="deleteBudgetEntry(${e.id})" title="Delete">×</button> </div>`;
};
list.innerHTML = entries.slice(0, shown).map(renderEntry).join(’’);
if (remaining > 0) {
list.innerHTML += `<button onclick="showMoreEntries()" style=" width:100%;padding:12px;margin-top:6px;background:var(--paper-2); border:1px solid var(--rule);border-radius:var(--r-sm);cursor:pointer; font-size:0.8rem;font-weight:600;color:var(--ink-3);font-family:inherit; transition:background 0.15s,color 0.15s; " onmouseover="this.style.background='var(--paper-3)';this.style.color='var(--ink)'" onmouseout="this.style.background='var(--paper-2)';this.style.color='var(--ink-3)'"> Show ${remaining} more entr${remaining === 1 ? 'y' : 'ies'} </button>`;
} else if (entries.length > 5) {
list.innerHTML += `<button onclick="showLessEntries()" style=" width:100%;padding:10px;margin-top:6px;background:none; border:none;cursor:pointer;font-size:0.75rem;color:var(--ink-3);font-family:inherit; ">Show less ↑</button>`;
}
}
...
summaryEl.innerHTML = rows.map(([label, val, sub]) => ` <div class="health-metric-row" style="flex-direction:column;align-items:flex-start;gap:2px;padding:8px 0"> <div style="display:flex;justify-content:space-between;width:100%"> <span class="hm-label">${label}</span> <span style="font-family:'IBM Plex Mono',monospace;font-size:0.88rem;font-weight:600;color:var(--ink)">${val}</span> </div> <span style="font-size:0.72rem;color:var(--ink-3)">${sub}</span> </div>`).join(’’);
}
...
list.innerHTML = debts.map(d => ` <div class="entry-item"> <div class="entry-cat-dot" style="background:var(--red)"></div> <div> <div class="entry-desc">${escHtml(d.name)}</div> <span class="entry-cat-label">${d.rate > 0 ? d.rate + '% p.a.' : 'No interest'}</span> </div> <div class="entry-amount" style="color:var(--red)">-${fmt(d.balance)}</div> <button class="entry-del" onclick="deleteDebt(${d.id})" title="Delete">×</button> </div>`).join(’’);
...
return ` <div class="entry-item"> <div class="entry-cat-dot" style="background:${dotColor}"></div> <div> <div class="entry-desc">${escHtml(a.name)}${statusBadge}</div> <span class="entry-cat-label">${catLabel} · every ${daySuffix(a.day)} of the month</span> </div> <div class="entry-amount" style="color:var(--red)">-${fmt(a.amount)}</div> <button class="entry-del" onclick="deleteAccrued(${a.id})" title="Delete">×</button> </div>`;
}).join(’’);
}
